...
def extract_doc2lines(gray_img, is_cleaned=True, raw_img=np.array([])):
    """"
    The following function take as input a document (image or pdf) and give back all the extracted lines as images
    input > gray_img : 2D array
    output > extracted_lines : list of 2D arrays
    """
...
def extract_words2char(word_img):
    ...
    for index in range(number_of_expected_char):
        ...
        char_img = cv2.copyMakeBorder(char_img,3,3,3,3, borderType=cv2.BORDER_CONSTANT
                                                        ,value=[255,255,255])
        #Expand the boudaries a little bit on the left and right
