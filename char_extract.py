...
# hence the aditionnal for loop running through extracted_line
extracted_char = []

for line_img in extracted_line :
    x_sum_img = np.sum(line_img,axis=0)/255
    ...
    for index in range(number_of_expected_char):
        ...
        #Expand the boudaries a little bit on the left and right
        extracted_char.append(line_img[ : , left_bound -2 :right_bound +2])

    print(len(extracted_char))
